...
```{r}
set.seed(200)
music_7NN<-  knn(train = music_data_train[, c("acousticness", "energy", "instrumentalness","liveness","tempo")],#<- training set cases # the variables we want to look at OTHER than danceability (i'm pretty sure)
               test = music_data_test[, c("acousticness", "energy", "instrumentalness","liveness","tempo")],    #<- test set cases
...
```{r}
music_eval <-(predict(music_7NN,newdata = music_data_test))
```